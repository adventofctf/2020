...
var secret = process.env.JWT_SECRET || "ADVENT_OF_CTF_2020"; // super secret
...
var u = { un: 'user', pw: 'incorrect', role: 'user' };
...
function administrator(res, token, role) {
  res.writeHead(200, {
  ...
  if(!decoded || !decoded.auth) {
    ...
    db.get(decoded.auth, function (err, record) {
      ...
      if (err || !r.valid) {
        ...
        administrator(res, token, decoded.role);
        return callback(res);
      }
    });
  }
